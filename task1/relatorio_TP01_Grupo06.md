...
### P9 — Uso de IA nos Simulados: Desempenho e Calibração de Expectativa

**Pergunta analítica:** Alunos que usaram IA nos simulados foram melhor na prova? E superestimaram mais ou menos a própria nota?

**Gráfico:** dois box plots lado a lado, comparando os mesmos três grupos (Fez sem IA / Usou IA em parte / Usou IA em todos) em duas dimensões: nota real e `erro_esperado` (nota esperada − nota real).

**Por que dois painéis?** A pergunta tem duas dimensões inseparáveis: desempenho objetivo e autopercepção. Um único gráfico não permitiria ver se a IA afetou a *calibração* da expectativa — se gerou confiança realista ou falsa confiança. O painel direito conecta diretamente à análise de P5.

**Achados:** O resultado vai contra a expectativa. Quem usou IA em *todos* os simulados teve boa nota (μ=11,6 pts) e a *menor* superestimativa do grupo (erro médio +1,27 pts). Quem fez sem IA teve a maior nota média (μ=11,9 pts), mas também a maior superestimativa (+2,36 pts). O grupo que misturou estratégias teve a pior nota (μ=10,1 pts) e superestimativa intermediária. Uma hipótese: a IA fornece explicações imediatas dos erros durante o simulado, calibrando melhor a autopercepção — porém os grupos são pequenos (n≈11–16) e não permitem conclusão definitiva.

---

### P10 — Tempo de Prova vs. Desempenho

**Pergunta analítica:** Alunos que ficaram mais tempo na prova obtiveram notas maiores?

**Gráfico:** scatter plot de `tempo_prova_min` vs `Nota Questões`, com linha de regressão e pontos coloridos por turma.

**Achados:** A correlação é praticamente nula (r = 0,019), confirmando o padrão já observado em P4 (horas de estudo, r = −0,06): nem o tempo de preparação nem o tempo de execução preveem o desempenho. T1 ficou em média 6 minutos a mais na sala que T2 (85,8 vs 79,9 min), mas T2 teve nota média maior. Ficar mais tempo na prova não significou revisar mais ou acertar mais questões — o domínio do conteúdo parece o fator determinante, independentemente do tempo investido.
